...
| 服务 | 地址 |
|------|------|
| 博客前台 | http://your_server_ip/ |
| 后台管理 (blog-admin) | http://your_server_ip/admin/ |
| 管理端 (manager-end) | http://your_server_ip/manager/ |
| API接口 | http://your_server_ip/api/v1/ |
| Swagger文档 | http://your_server_ip/api/v1/swagger-ui.html |
...
```bash
ssh Tencent "mysql -u root -pyour_db_password blog_db" << 'EOF'
-- 用户表
...
server {
    ...
    location /environment/api/ {
        proxy_pass http://your_backend_ip:8083/environment/api/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
```bash
# 登录数据库
ssh Tencent "mysql -u root -pyour_db_password blog_db"

# 备份数据库
ssh Tencent "mysqldump -u root -pyour_db_password blog_db > /var/www/blog/backup_$(date +%Y%m%d).sql"

# 恢复数据库
ssh Tencent "mysql -u root -pyour_db_password blog_db < /var/www/blog/backup.sql"
```
...
# 检查数据库连接
ssh Tencent "mysql -u root -pyour_db_password -e 'SELECT 1'"
```
